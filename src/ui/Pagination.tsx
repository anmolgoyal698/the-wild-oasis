...
const Pagination = ({count}) => {
  ...
  const curPage = searchParams.get("page") ? parseInt(searchParams.get("page")) : 1;
  ...
  return (
    <StyledPagination>
      <P>
        Showing <span>{(curPage - 1) * PAGE_SIZE + 1}</span> to <span>{Math.min(curPage * PAGE_SIZE, count)}</span> of <span>{count}</span> results
      </P>
      <Buttons>
